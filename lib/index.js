...
const svg2htmlLib = {
  ...
  svg2html({ inDir, outDir }) {
    ...
    const folders = dirents.filter((dirent) => {
      ...
      .filter(name => !this.exclude.split('|').includes(name));

    // DLR
    this.createHtml(svgs, inDir, outDir);
  ...
  createHtml(svgs, inDir, outDir) {
    this.generateAllHtml(inDir, outDir).then(() => {
      return this.generateIndexHtml(svgs, inDir, outDir);
    }).then(() => {
      spinner.stop();
    });
  },

  buildHtml(svgs, folder) {
  ...
  generateIndexHtml(svgs, inDir, outDir) {
    if (svgs.length) {
      const allSvgMap = this.buildSvgMap(svgs);

      return this.buildHtml(allSvgMap, inDir).then((html) => {
  ...
  generateAllHtml() {
    if (!this.travelAll) {
      ...
      return this.globPromise(`${this.inDir}/**/*.svg`).then((files) => {
        ...
        return this.buildHtml(allSvgMap, this.inDir).then((html) => {
